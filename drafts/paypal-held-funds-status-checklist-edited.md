...
Если PayPal показывает `held`, `on hold`, `pending`, `reserve` или `unavailable`, одного слова недостаточно, чтобы установить причину, срок или итог. Один и тот же поисковый запрос может скрывать разные состояния: отдельный платёж, резерв, ограничение аккаунта, проверку операции, спор или вывод. Официальные соглашения и справка PayPal рассматривают их отдельно, а точные действия зависят от рынка, типа аккаунта и текста уведомления. [Claims: OF-01–OF-04, OF-08, OF-13; Sources: S1, S2, S4, S5]
